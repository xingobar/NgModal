module.exports.NgModal = (function() {
	var m,
		k,
		c,
		e,
		...
		n = function(a) {
			...
			0 == $('#ng_modal_outside').length &&
				...
				(m = a.hasOwnProperty('animate') ? a.animate : 'ng-rotate'),
				(k = m + '-end'),
				c.addClass(m));
		...
		p = function(a) {
			if (a) {
				...
				if (a.hasOwnProperty('time'))
					if ('number' === typeof a.time) r(a.time);
					else throw Error('params time is not integer');
				else r(5e3);
				if (a.hasOwnProperty('shadowClose'))
					if ('boolean' === typeof a.shadowClose) a.shadowClose && t();
				...
				if (a.hasOwnProperty('draggable'))
					if ('boolean' === typeof a.draggable) a.draggable && u();
				...
				if (a.hasOwnProperty('closeBtn'))
					if ('boolean' === typeof a.closeBtn)
						a.closeBtn &&
							(l.append('<a href="#" class="ng-modal-title-close"></a>'),
							$('.ng-modal-title-close').click(function() {
								g();
							}));
					else throw Error('params closeBtn must be boolean');
			}
			v();
		},
		v = function() {
			if (navigator.userAgent.match(/Android/i) || navigator.userAgent.match(/iPhone/i))
				b(),
					320 < window.innerWidth ? c.css({ width: '20rem' }) : c.css({ width: '18rem' }),
					h.find('button').each(function(a, b) {
						$(b).css({ width: '8rem' });
					});
		},
		q = function(a) {
